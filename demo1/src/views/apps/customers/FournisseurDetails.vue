<template>
        ...
        <div class="card mb-5 mb-xl-8">
          ...
          <div class="card-body pt-15">
            ...
            <div id="kt_customer_view_details" class="collapse show"     v-if="fournisseur">
              <div class="py-5 fs-6">
                <!--begin::Badge-->
                <div class="badge badge-light-info d-inline">{{ fournisseur.country }}</div>
                <!--begin::Badge-->
                <!--begin::Details item-->
                <div class="fw-bold mt-5">Account ID</div>
                <div class="text-gray-600">ID-{{ fournisseur.id }}</div>
                ...
                <div class="text-gray-600">
                  {{ fournisseur.localisation }} <br />{{ fournisseur.country }}  
                            </div>
                <!--begin::Details item-->
                <!--begin::Details item-->
                <div class="fw-bold mt-5">Number</div>
                <div class="text-gray-600">{{ fournisseur.numerotel }}</div>
                <!--begin::Details item-->
                <!--begin::Details item-->
                <div class="fw-bold mt-5">Horaire</div>
                <div class="text-gray-600">{{ fournisseur.horaire }}</div>
                <!--begin::Details item-->
                <!--begin::Details item-->
    ...
    >
      ...
      <div class="menu-item px-5">
        <a @click="SuppAdmin" class="menu-link text-danger px-5"> Supprimer le Compte </a>
      </div>
      <div class="menu-item px-5">
        <a @click="AprvFrnsr" class="menu-link text-success px-5"> Approuver le Compte </a>
      </div>
            </div>          <!--end::Menu-->
          </li>
          <!--end:::Tab item-->
        ...
        <div class="fv-row mb-10">
          ...
          <input
            class="form-control form-control-lg form-control-solid"
            type="text"
            v-model="fournisseur.localisation"
          />
          <!--end::Input-->
          <div class="fv-plugins-message-container">
            <div class="fv-help-block">
  ...
  import {deleteFournisseur,approuverFournisseur,getFournisseurById ,type  Fournisseur } from "@/core/data/customers";
  import {updateFournisseur,getClientsByAdmin ,type Client}from "@/core/data/customers";
  ...
  const fournisseur = ref<Fournisseur>({} as Fournisseur);
   ...
   //get admin by id with axios
      ...
      onMounted(async () => {
          ...
          const response =await getFournisseurById(adminId);
          fournisseur.value = response;
  
          const adminResponse = await getFournisseurById(adminId);
        ...
        const saveChanges = () => {    
  console.log(fournisseur.value.admin_id)
          updateFournisseur(fournisseur.value,fournisseur.value.id²).then((response) => {
      Swal.fire({
    ...
    if (sort.label) {
      ...
      const SuppAdmin = () => { 
        ...
        deleteFournisseur(adminIdToDelete)
        .then((response) => {
      Swal.fire({
        text: "Le profil de '" + fournisseur.value?.nom + "' a supprimée avec succès.",
            icon: "success",
            confirmButtonText: "Ok",
            buttonsStyling: false,
            heightAuto: false,
    ...
    window.location.href = 'dashboard#/admin/List';
      ...
      const AprvFrnsr = () => { 
        ...
        approuverFournisseur(adminIdToDelete)
        .then((response) => {
      Swal.fire({
        text: "Le Fournisseur  de '" + fournisseur.value?.nom + "' est approuvé avec succès.",
            icon: "success",
            confirmButtonText: "Ok",
            buttonsStyling: false,
            heightAuto: false,
    ...
    window.location.href = 'dashboard#/fournisseur/list';
