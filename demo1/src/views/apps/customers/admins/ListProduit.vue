    ...
    <div class="col-md-4" v-for="(customer, index) in customersArray" :key="index">
      <div class="card mb-4">
        ...
        <div class="card-body">
          ...
          <input type="number" v-model="customer.quantite" class="form-control" id="quantity" min="1" style="width:60px;">
        </div>
          <button @click="comander(customer)" class="btn btn-success">✅Commander</button>
        </div>
      </div>
    </div>
  </div>
<div>
  ...
  import {commanderProduit, getProduits, type Commande, type ICustomer } from "@/core/data/customers";
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const selectedIds = ref<Array<number>>([]);
        ...
        const responsed =  store.getAdminData().then((response) => {
      ...
      const comander=(produit:object)=>{
        const formData=new FormData();
        formData.append('produit_id',produit.id);
        formData.append('quantite',produit.quantite);
        formData.append('admin_id',admin.value.id);
         
        commanderProduit(formData).then((response) => {
