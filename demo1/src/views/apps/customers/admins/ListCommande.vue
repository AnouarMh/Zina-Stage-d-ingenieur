    ...
    <div class="card-body pt-0">
      ...
      <template v-slot:produit="{ row: customer }">
        ...
        <template v-slot:action="{ row: customer }">
            ...
            <button  class="btn btn-danger" @click="annulerrCommande(customer.id)" >✖️ Annuler</button>
...
import {annulerCommande, getCommandes,type Commande } from "@/core/data/customers";
    ...
    const selectedIds = ref<Array<number>>([]);
      ...
      onMounted(async () => {
       ...
       const id=(admin.value.id)
        const response =await getCommandes(id);
...
/*
    ...
    const annulerrCommande =  (id: number) => {
      console.log(id)
      annulerCommande(id).then((response) => {
