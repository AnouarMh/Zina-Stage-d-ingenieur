...
class AdminController extends Controller
{
   ...
   //get les clients par centre
    ...
    //get client by id 
    public function getClientByAdmin($id)
    {
        $client = Client::find($id);
        //if client.approuver= 0 return approuver=false 
        if($client->approuver==0){
            $client->approuver=false;
        }
    ...
    {
        $client = Client::find($id);
        $client->approuver=true;
        $client->save();
        if($client->approuver==1){
            $client->approuver=true;
        }
        return response()->json( $client, 200);
    }
